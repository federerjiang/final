...
# NN_MODEL = "/Users/federerjiang/research-project/aitrans-competition/final/a2c/final-result/room-hml/actor.pt-1553"
NN_MODEL_2 = "/Users/federerjiang/research-project/aitrans-competition/final/a2c/final-result/room-x/actor.pt-133"
...
class Algorithm:
	...
	def run(self, time, S_time_interval, S_send_data_size, S_chunk_len, S_rebuf, S_buffer_size, S_play_time_len, S_end_delay, S_decision_flag, S_buffer_flag, S_cdn_flag, end_of_video, cdn_newest_id, download_id, cdn_has_frame, IntialVars):
		...
		# low_flag = False
		# if np.mean(self.frame_thps) < 1 and (np.std(self.frame_thps) < 0.1 or np.max(self.frame_thps)<1.1):
		if np.mean(self.frame_thps) < 0.85 and np.std(self.frame_thps) < 0.095:
