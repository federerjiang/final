...
class Actor(nn.Module):
	...
	def forward(self, inputs, batch_size=1):
		...
		split_4 = F.relu(self.a_fc4(inputs[:, 1:2, -1]))
		split_5 = F.relu(self.a_conv5(inputs[:, 4:5, 0:16])).view(batch_size, -1)
		split_6 = F.relu(self.a_conv6(inputs[:, 5:6, 0:16])).view(batch_size, -1)
		...
		split_4 = F.relu(self.c_fc4(inputs[:, 1:2, -1]))
		split_5 = F.relu(self.c_conv5(inputs[:, 4:5, 0:16])).view(batch_size, -1)
		split_6 = F.relu(self.c_conv6(inputs[:, 5:6, 0:16])).view(batch_size, -1)
